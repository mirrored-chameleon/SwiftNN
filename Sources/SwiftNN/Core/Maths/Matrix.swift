...
public struct Matrix<T: FloatingPoint & Codable>: Codable {
    public var rows: Int
    ...
    public static func dot(_ a: Matrix<T>, _ b: Matrix<T>) -> Matrix<T> {
        precondition(a.columns == b.rows)

        var result = Matrix<T>(
            rows: a.rows,
            columns: b.columns,
            grid: Array(repeating: 0, count: a.rows * b.columns),
        )
        ...
        for i in 0 ..< a.rows {
            for j in 0 ..< b.columns {
                ...
                for k in 0 ..< a.columns {
                    sum =
                        sum +
                        a.grid[i * a.columns + k] *
                        b.grid[k * b.columns + j]
                }
